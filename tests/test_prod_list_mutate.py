...
def start_server() -> subprocess.Popen:
    ...
    log = Path(f"/tmp/test-prod-list-mutate-{PORT}.log")
...
def main() -> None:
    ...
    try:
        with socket.create_connection(("127.0.0.1", PORT), timeout=2) as sock:
            assert call(sock, "RPUSH", "l", "a", "b", "a", "c", "a") == 5
            assert call(sock, "LREM", "l", "2", "a") == 2
            assert call(sock, "LRANGE", "l", "0", "-1") == ["b", "c", "a"]
            assert call(sock, "LREM", "l", "0", "a") == 1
            assert call(sock, "LRANGE", "l", "0", "-1") == ["b", "c"]
            assert call(sock, "RPUSH", "t", "0", "1", "2", "3", "4") == 5
            assert call(sock, "LTRIM", "t", "1", "3") == "OK"
            assert call(sock, "LRANGE", "t", "0", "-1") == ["1", "2", "3"]
            assert call(sock, "LTRIM", "t", "5", "10") == "OK"
            assert call(sock, "EXISTS", "t") == 0
            print("PASS LREM/LTRIM")
            print("test_prod_list_mutate: ALL PASSED")
    finally:
        proc.terminate()
        try:
            proc.wait(timeout=3)
